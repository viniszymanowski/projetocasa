...
def main():
    ...
    try:
        dados = ler_dados_arquivo(caminho_csv)
        resultado, checks = checar_conformidade(dados)

        # Caminho do relatório (sempre na raiz do projeto)
        raiz_projeto = caminho_csv.resolve().parents[2]
        caminho_relatorio = raiz_projeto / "docs" / "normas" / "relatorio_conformidade.md"

        gerar_relatorio(resultado, checks, caminho_relatorio)
